...
/**
 * SinaFavoritesAdaptor
 *
 * @version
 * @author 
 */
class SinaFavoritesAdaptor {
	...
	public static Status createStatus(String jsonString) throws LibException {
		try {
			...
			return createStatus(statusJson);
	...
	public static ArrayList<Status> createStatusList(String jsonString) throws LibException {
		try {
			...
			for (int i = 0; i < size; i++) {
				JSONObject jsonFavorite = jsonList.getJSONObject(i);
				statusList.add(createStatus(jsonFavorite.getJSONObject("status")));
			}
